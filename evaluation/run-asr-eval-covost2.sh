...
cer=false
sclite=sclite
hyp_asr=/exp/cxiao/scale23/hf_data_covost2/fr/test.text
arabic=false
dset=test
...
_opts=""
cat $data_base_dir/${testset}.src.stm >"${test_score_dir}/data/${testset}.src.stm"

# Convert the hypothesis file to STM format
pyscripts/utils/text2stm_covost2.py \
    -i "${hyp_asr}" \
    -o "${test_score_dir}/data/_hyp.stm" \
    -r "${test_score_dir}/data/${testset}.src.stm" \
    --dset ${dset} ${_opts}
...
./run_scale23_evals.sh \
    ...
    --ref_asr "${test_score_dir}/data/${testset}.src.stm" \
    --arabic "${arabic}" \
    --python "${python}" \
    --no_glm "${no_glm}" \
    --cer "${cer}" \
