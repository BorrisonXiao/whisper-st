...
inference_nj=24      # Number of jobs for decoding, note that each job will use a GPU
merge_decode=true    # Whether to merge the utterances at decoding time

# If the language is Korean, set eval_cer to true, otherwise false
eval_cer=false
...
min_duration=0.0
start_at_zero=true
if "${merge_utt}" || "${merge_decode}"; then
