...
org_hf_datadir=dump/hf_data # Directory to the original hugging face dataset.
...
pseudo_st=false             # Whether to use pseudo-translation for training
stm_dir=                    # The path to the directory containing the STM files
eval_checkpoint=            # The checkpoint to evaluate
num_beams=1                 # Number of beams for decoding
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    log "Stage 1: Run ASR finetuning on the training data"
    _langcode=${src_lang}
    if [ -n "${dialect}" ]; then
        _langcode="${dialect}"
    fi
    _dir="${PWD}/${st_exp}/${_langcode}/${train_set}/asr/${peft_method}"
    _logdir="${_dir}/logdir"
    mkdir -p "${_logdir}"
    ...
    if [ -n "${speed_perturb_factors}" ]; then
        opts+=" --speed-perturb-factors ${speed_perturb_factors} "
    fi
    ...
    if "${precompute_feats}"; then
        ...
        if [ ! -d "${hf_datadir}/features/${_feat_type}/${src_lang}.${train_set}.asr" ] ||
            ...
            ${cuda_cmd} --gpu 1 "${_logdir}"/fe_${JOBID}.log \
    ...
    if "${fe_only}"; then
        ...
        log "Using ASR config: ${asr_config}"
        ...
        if "${debug}"; then
            ...
            ${cuda_cmd} --gpu ${ngpu} "${_logdir}"/finetune_${JOBID}.log \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    log "Stage 2: Run (distributed) ASR inference on the dev/test data."
    _lang="${src_lang}"
    if [ -n "${dialect}" ]; then
        _lang="${dialect}"
    fi
    ...
    for dset in ${valid_set} ${extra_valid_set} ${test_sets}; do
        ...
        _logdir="${st_exp}/logdir/inference_asr/${_lang}/${train_set}/${dset}/${peft_method}"
        mkdir -p "${_logdir}"

        _dir="${st_exp}/${_lang}/decode/${train_set}/${dset}/asr/${peft_method}"
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    log "Stage 4: Run ST finetuning on the training data"
    ...
    if "${precompute_feats}"; then
        ...
        if [ ! -d "${hf_datadir}/features/${_feat_type}/${src_lang}.${train_set}.st" ] ||
            [ ! -d "${hf_datadir}/features/${_feat_type}/${src_lang}.${dev_name}.st" ]; then
            if "${debug}"; then
                ${python_hf} ${train_tool} \
                    --feat-extraction \
                    --train-set ${train_set} \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    log "Stage 5: Run (distributed) ST inference on the dev/test data."
    ...
    # for dset in ${valid_set} ${extra_valid_set}; do
    for dset in ${test_sets}; do
        _logdir="${st_exp}/logdir/inference_st/${src_lang}/${train_set}/${dset}/${peft_method}"
        mkdir -p "${_logdir}"
        if "${merge_decode}"; then
            ...
            _dsetdir=${stm_dir}
            pyscripts/utils/stm2keys.py \
                -i "${_dsetdir}" \
                -o "${_logdir}/tmp" \
                -s "${dset}"
        fi

        _dir="${st_exp}/${src_lang}/decode/${train_set}/${dset}/st/${peft_method}"
        _modeldir="${st_exp}/${src_lang}/${train_set}/st/${peft_method}"

        key_file=${_logdir}/tmp/keys.scp
        # 1. Split the key file
        _nj=$(min "${inference_nj}" "$(wc <${key_file} -l)")
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    log "Stage 6: Run evaluation on the ST decoded data."
    ...
    for dset in ${test_sets}; do
        ...
        ${eval_script} \
            ...
            --data_base_dir ${stm_dir} \
            --dset "${_dset}" \
            --score_dir scores_ft/st/hf_whisper_${model_name}/${src_lang}/${peft_method}/${train_set}/${dset} \
            --framework "${framework}" ${opts}
        cd -
    done
fi
