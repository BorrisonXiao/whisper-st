...
def main():
    ...
    if args.segments is not None:
        ...
        with out_num_samples.open("w") as fnum_samples:
            if args.save_wav:
                Path(args.wavdir / "data").mkdir(parents=True, exist_ok=True)
                wavscppath = Path(args.wavdir) / "wav.scp"
                wavscpout = wavscppath.open("w")
            for uttid, (rate, wave) in tqdm(loader):
                ...
                if args.save_wav:
                    owavpath = str(args.wavdir / "data" / f"{uttid}.wav")
                    soundfile.write(owavpath, wave, rate)
                    wavscpout.write(f"{uttid} {owavpath}\n")
