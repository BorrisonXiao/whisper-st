...
def parse_stm_filename(stm_file_path: str) -> Tuple[str, str, str, str]:
    ...
    task, src_lang_tgt_lang, split = stm_file_name.split('.')
    src_lang, tgt_lang = src_lang_tgt_lang.split('-')
    return task, src_lang, tgt_lang, split
...
def get_transcript_translation_dict(stm_file: Tuple[str, str]) -> Dict[str, Tuple[str, str, str, str, str]]:
    ...
    # parse_stm_filename
    task, src_lang, tgt_lang, split = parse_stm_filename(st_file)
...
def get_trans_dict(stm_file: Tuple[str, str]) -> Dict[str, Tuple[str, str, str, str]]:
    ...
    # parse_stm_filename
    task, src_lang, tgt_lang, split = parse_stm_filename(stm_file)
...
def process_stm_file_pair(stm_file_pair, raw_data_location, output_path, data_files=DATA_FILES):
    ...
    task, src_lang, tgt_lang, split = parse_stm_filename(st_file)
    dataset_path = f"{output_path}/{src_lang}.{split}/"

    if f"{src_lang}.{split}" not in data_files and not os.path.exists(dataset_path) or os.path.exists(dataset_path) and not os.listdir(dataset_path):
        ...
        task, src_lang, tgt_lang, split = parse_stm_filename(
            st_file)
        ...
        print(
            f"Skipping {src_lang}.{split} as it already exists or it's in the skipping list.")
...
def create_stm_file_pair(stm_files):
    ...
    for stm_file in stm_files:
        ...
        if task == "st":
            ...
            sr_file = f"{_dir}/sr.{src_lang}-{src_lang}.{split}.stm"
