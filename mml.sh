...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    if [ ! -d "${org_hf_datadir}/features/${_feat_type}/${src_lang}.${mt_train_set}.mt" ] ||
        ...
        log "${org_hf_datadir}/features/${_feat_type}/${src_lang}.${mt_train_set}.mt or ${org_hf_datadir}/features/${_feat_type}/${src_lang}.${extra_valid_set}.mt does not exist..."
        if "${debug}"; then
    ...
    ${cuda_cmd} JOB=1:1 "${_logdir}"/concatenate_features.log \
        ...
        --dset2 ${org_hf_datadir}/features/${_feat_type}/${src_lang}.${mt_train_set}.mt \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        if "${debug}"; then
            ${inference_tool} \
                --keyfile ${_logdir}/decode.4.scp \
                --src-lang ${src_lang} \
                --tgt-lang ${src_lang} \
                --output_dir ${_logdir}/output.4 \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        if [ "${dset}" = "${valid_set}" ] || [ "${dset}" = "${extra_valid_set}" ]; then
            eval_script=run-devset-eval.sh
        elif [ "${dset}" = "fleurs_test" ] || [ "${dset}" = "europarl_test" ] || [ "${dset}" = "mtedx_test" ]; then
            eval_script=run-ood-eval.sh
        else
            eval_script=run-testset-eval.sh
        fi
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        _logdir="${st_exp}/logdir/inference_mml/asr/${_lang}/${train_set}/${dset}/${peft_method}${train_suf}${decode_suf}"
        mkdir -p "${_logdir}"
        if "${merge_decode}"; then
            # If dset is in test_sets, i.e. it contains the "_test" substring, add a suffix to the langdir
            if [[ ${dset} == *"_test" ]]; then
        ...
        _dir="${st_exp}/${src_lang}/decode/${train_set}/${dset}/mml/asr/${peft_method}_${batch_mask_prob}_${token_mask_prob}${train_suf}${decode_suf}"
        _modeldir="${st_exp}/${src_lang}/${train_set}/mml/${peft_method}_${batch_mask_prob}_${token_mask_prob}"
