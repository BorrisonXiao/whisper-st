...
def inference(keyfile, dset, src_lang, tgt_lang, output_dir, model_name, pretrained_model=None, peft_model=None, task="transcribe", batch_size=1):
    ...
    # Here we enforce the batch size to be 1 for adding the ASR transcript to the decoder prompt
    batch_size = 1
    ...
    with open(output_st, "w") as f_st:
        ...
        for utt in ds:
            ...
            # Use the gold ASR transcript as the prompt
            transcript = batch[0]["transcript"]
            transcript_ids = processor.tokenizer([transcript], add_special_tokens=False).input_ids[0]
            _forced_decoder_ids = forced_decoder_ids + [(i + len(forced_decoder_ids) + 1, id) for i, id in enumerate(transcript_ids)] + [(len(forced_decoder_ids) + len(transcript_ids) + 1, startoftranslation_id)]
            # Generate token ids
            predicted_ids = model.generate(
                input_features, forced_decoder_ids=_forced_decoder_ids)
            ...
            for i, st_hyp in enumerate(st_hyps):
