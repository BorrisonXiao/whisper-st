...
def merge_utts(src_lang, tgt_lang, input_base_dir, output_base_dir, splits=SPLITS, num_outputs=1, t=30):
    """
    Merge the utterances in order to the closest t (default 30) seconds for efficient training and inference.
    """
    ...
    for split in splits:
        ...
        for i, (uttid, utt) in enumerate(asr_utts_dict.items()):
            if cur_dur + utt.dur > t or cur_root_fname is not None and cur_root_fname != utt.root_filename:
