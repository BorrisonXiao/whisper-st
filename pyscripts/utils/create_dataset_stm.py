...
from datasets import Dataset, Features, Audio, Value, concatenate_datasets
...
def parse_stm_filename(stm_file_path: str) -> Tuple[str, str, str, str]:
    ...
    Example:
        >>> stm_file_path = "./st.bem-eng.bigc.dev.stm"
        >>> parse_stm_filename(stm_file_path)
        ("st", "bem", "eng", "bigc", "dev")
    """
    stm_file_name = os.path.splitext(os.path.basename(stm_file_path))[0]
    task, src_lang_tgt_lang, dataset, split = stm_file_name.split('.')
    src_lang, tgt_lang = src_lang_tgt_lang.split('-')
    return task, src_lang, tgt_lang, dataset, split
...
def find_stm_files(directory: str, mode: str, dset: str) -> List[Tuple[str, str]]:
    ...
    """
    # Find all stm files in the directory (including subdirectories)
    _stm_files = [os.path.join(root, file) for root, _, files in os.walk(
        directory) for file in files if file.endswith(".stm")]

    stm_files = [file for file in _stm_files if Path(file).stem.startswith(
        f"{mode}.")] if mode == "sr" or mode == "st" else _stm_files

    if dset is not None:
        stm_files = [
            file for file in stm_files if f".{dset}." in Path(file).stem]

    # Create a list of tuples with each tuple containing the sr and st file from the same dataset
    stm_files_list = []
    for stm_file in stm_files:
        stm_files_list.append(stm_file)
...
def get_transcript_translation_dict(stm_file: Tuple[str, str]) -> Dict[str, Tuple[str, str, str, str, str]]:
    ...
    # parse_stm_filename
    task, src_lang, tgt_lang, dataset, split = parse_stm_filename(st_file)
...
def get_trans_dict(stm_file: Tuple[str, str]) -> Dict[str, Tuple[str, str, str, str]]:
    """ Return a dictionary with utterance id as the key and values are src_lang, tgt_lang, wav_file and translation.
    """
    trans_dict = dict()
    print(f"Processing following stm file: {stm_file}")
    st_dict = read_stm_file(stm_file)
    # parse_stm_filename
    task, src_lang, tgt_lang, dataset, split = parse_stm_filename(stm_file)

    for utterance_id, (wav_file, trans) in st_dict.items():
        trans_dict[utterance_id] = (
            src_lang, tgt_lang, wav_file, trans)
...
def preprocess_for_hf_dataset_st(translation_dict: Dict[str, Tuple[str, str, str, str]], raw_data_location: str):
    """ Load audio from the given dictionary and create a HuggingFace dataset.
    """
    audio_files, utterance_ids, translations, src_langs, tgt_langs = [], [], [], [], []
    for utterance_id, (src_lang, tgt_lang, wav_file, translation) in tqdm(translation_dict.items()):
        audio_files.append(os.path.join(raw_data_location, wav_file))
        utterance_ids.append(utterance_id)
        translations.append(translation)
        src_langs.append(src_lang)
    ...
    return {"audio": audio_files, "uttid": utterance_ids, "translation": translations, "src_lang": src_langs, "tgt_lang": tgt_langs}


def preprocess_for_hf_dataset_sr(trans_dict: Dict[str, Tuple[str, str, str, str]], raw_data_location: str):
    """ Load audio from the given dictionary and create a HuggingFace dataset.
    """
    breakpoint()
    audio_files, utterance_ids, transcripts, src_langs, tgt_langs = [], [], [], [], []
    for utterance_id, (src_lang, tgt_lang, wav_file, transcript) in tqdm(trans_dict.items()):
        audio_files.append(os.path.join(raw_data_location, wav_file))
        utterance_ids.append(utterance_id)
        transcripts.append(transcript)
        src_langs.append(src_lang)
    ...
    return {"audio": audio_files, "uttid": utterance_ids, "transcript": transcripts, "src_lang": src_langs, "tgt_lang": tgt_langs}


def create_hf_dataset(dataset, transcript_file, mode="all"):
    """
    ...
    if mode == "all" or mode == "sr":
        feat_dict["transcript"] = Value(dtype="string")
    if mode == "all" or mode == "st":
        feat_dict["translation"] = Value(dtype="string")
...
def process_stm_file_pair(stm_file_pair, raw_data_location, output_path, data_files=DATA_FILES):
    ...
    task, src_lang, tgt_lang, dataset_name, split = parse_stm_filename(st_file)
    dataset_path = f"{output_path}/{src_lang}.{dataset_name}.{split}/"

    if f"{src_lang}.{dataset_name}.{split}" not in data_files and not os.path.exists(dataset_path) or os.path.exists(dataset_path) and not os.listdir(dataset_path):
        ...
        task, src_lang, tgt_lang, dataset_name, split = parse_stm_filename(
            st_file)
        ...
        print(
            f"Skipping {src_lang}.{dataset_name}.{split} as it already exists or it's in the skipping list.")


def process_stm_file_st(stm_file, raw_data_location, output_path, data_files=DATA_FILES, save_to_disk=True):
    # create a huggingface dataset from the stm file
    task, src_lang, tgt_lang, dataset_name, split = parse_stm_filename(
        stm_file)
    dataset_path = f"{output_path}/{src_lang}.{dataset_name}.{split}/"
    ...
    if f"{src_lang}.{dataset_name}.{split}" not in data_files and not os.path.exists(dataset_path) or os.path.exists(dataset_path) and not os.listdir(dataset_path):
        # combine the sr and st files into a dictionary
        translation_dict = get_trans_dict(stm_file)

        dataset = preprocess_for_hf_dataset_st(
            translation_dict, raw_data_location)

        hf_dataset = create_hf_dataset(dataset, stm_file, mode="st")
    ...
    return ("st", hf_dataset, dataset_path)


def process_stm_file_sr(stm_file, raw_data_location, output_path, data_files=DATA_FILES, save_to_disk=True):
    # create a huggingface dataset from the stm file
    task, src_lang, tgt_lang, dataset_name, split = parse_stm_filename(
        stm_file)
    dataset_path = f"{output_path}/{src_lang}.{dataset_name}.{split}/"
    ...
    if f"{src_lang}.{dataset_name}.{split}" not in data_files and not os.path.exists(dataset_path) or os.path.exists(dataset_path) and not os.listdir(dataset_path):
        ...
        dataset = preprocess_for_hf_dataset_sr(
            translation_dict, raw_data_location)

        hf_dataset = create_hf_dataset(dataset, stm_file, mode="sr")
    ...
    return ("sr", hf_dataset, dataset_path)


def process_stm_file_umtl(stm_file, raw_data_location, output_path, data_files=DATA_FILES):
    # create a huggingface dataset from the stm file
    task, src_lang, tgt_lang, dataset_name, split = parse_stm_filename(
        stm_file)
    if task == "st":
        return process_stm_file_st(stm_file, raw_data_location, output_path, data_files, save_to_disk=False)
    elif task == "sr":
        return process_stm_file_sr(stm_file, raw_data_location, output_path, data_files, save_to_disk=False)


def create_stm_file_pair(stm_files):
    stm_files_list = []
    for stm_file in stm_files:
        task, src_lang, tgt_lang, dataset, split = parse_stm_filename(stm_file)
        if task == "st":
            _dir = os.path.dirname(stm_file)
            # Replace the /st/ with /asr/ to get the sr file
            _dir = _dir.replace("/st/", "/asr/")
            sr_file = f"{_dir}/sr.{src_lang}-{src_lang}.{dataset}.{split}.stm"
...
def main():
    ...
    parser.add_argument("--raw-data-location", type=Path,
                        default="raw_data",
                        help="Path to the raw data location.")
    parser.add_argument("--output-path", type=Path,
                        default="hf_data",
                        help="Path to the output location.")
    parser.add_argument("--src-lang", type=str, default="bem",
                        choices=["bem", "mlt", "bho",
                                 "mar", "que", "apc", "tmh"],
    ...
    parser.add_argument("--mode", type=str, default="all",
                        choices=["st", "sr", "all", "umtl"],
                        help="The supervision to process.")
    ...
    stm_files = find_stm_files(
        raw_data_location, mode=args.mode, dset=args.dset)
    if args.src_lang:
        stm_files = [
            stm_file for stm_file in stm_files if args.src_lang in Path(stm_file).stem]

    print(f"stm_files: {stm_files}")
    print(f"mode: {args.mode}")

    if args.mode == "all":
        stm_files = create_stm_file_pair(stm_files=stm_files)
        process_fun = process_stm_file_pair
    elif args.mode == "st":
        process_fun = process_stm_file_st
    elif args.mode == "sr":
        process_fun = process_stm_file_sr
    elif args.mode == "umtl":
        process_fun = process_stm_file_umtl
    else:
        raise ValueError(f"Invalid mode: {args.mode}")

    # Create a new function with raw_data_location and output_path pre-filled
    process_stm_func = partial(
        process_fun, raw_data_location=raw_data_location, output_path=output_path)

    hf_dsets = []
    with multiprocessing.Pool() as pool:
        max_ = len(stm_files)
        with tqdm(total=max_, desc=f"Creating HuggingFace datasets from wav and text files in {raw_data_location}") as pbar:
            for i, hf_dset in tqdm(enumerate(pool.imap_unordered(process_stm_func, stm_files))):
                pbar.update()
                hf_dsets.append(hf_dset)

    # for entry in stm_files:
    #     hf_dsets.append(process_stm_func(entry))

    if args.mode == "umtl":
        to_concat = {}
        for task, hf_dataset, dataset_path in hf_dsets:
            if Path(dataset_path).suffix == ".train":
                to_concat[dataset_path] = [
                    hf_dataset] if dataset_path not in to_concat else to_concat[dataset_path] + [hf_dataset]
            else:
                # For dev or test sets, only ST datasets are present as it will be the final objective
                if task == "st":
                    to_concat[dataset_path] = [
                        hf_dataset] if dataset_path not in to_concat else to_concat[dataset_path] + [hf_dataset]

        for dataset_path, datasets in to_concat.items():
            concatenate_datasets(datasets).save_to_disk(dataset_path)
