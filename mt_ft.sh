...
use_asr_prompt=false                      # Whether to mask the ASR hypothesis at MT training time
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    _dir="${st_exp}/${src_lang}/${train_set}/mt/${peft_method}_${min_sample_prob}_${max_sample_prob}"
    _logdir="${_dir}/logdir"
    mkdir -p "${_logdir}"
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    log "Stage 2: Run (distributed) MT inference on the dev/test data."
    decode_suf="_org"
    ...
    for dset in ${test_sets}; do
        ...
        _logdir="${st_exp}/logdir/inference_mt/${src_lang}/${train_set}/${dset}/${peft_method}${train_suf}${decode_suf}"
        mkdir -p "${_logdir}"
        if "${merge_decode}"; then
        ...
        _dir="${st_exp}/${src_lang}/decode/${train_set}/${dset}/mt/${peft_method}_${min_sample_prob}_${max_sample_prob}${train_suf}${decode_suf}"
        if "${promptless_decode}"; then
            _dir="${_dir}_promptless"
        elif "${use_asr_prompt_decode}"; then
            _dir="${_dir}_asr_prompt"
        fi
        _modeldir="${st_exp}/${src_lang}/${train_set}/mt/${peft_method}_${min_sample_prob}_${max_sample_prob}"
        ...
        if "${promptless_decode}"; then
            ...
            inference_tool="pyscripts/utils/hf_whisper_inference_mt.py"
        fi
    ...
    if ! "${promptless_decode}"; then
        if "${use_asr_prompt_decode}" || ! "${disable_asr_inference}"; then
            ...
            for dset in ${test_sets}; do
                ...
                score_dir=scores_ft/mt/asr/hf_whisper_${model_name}/${src_lang}/${peft_method}_${min_sample_prob}_${max_sample_prob}/${train_set}${train_suf}${decode_suf}/${dset}
                if "${promptless_decode}"; then
