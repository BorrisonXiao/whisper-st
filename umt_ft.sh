...
use_asr_prompt=true                       # Whether to mask the ASR hypothesis at MT training time, note this must be True for MT training
use_asr_prompt_dev=false                  # Whether to mask the ASR hypothesis at MT validation time note this must be False for MT validation
...
min_sample_prob=0.0                       # The minimum probability for sampling the MT ASR hypothesis
max_sample_prob=0.0                       # The maximum probability for sampling the MT ASR hypothesis (0.0 means disable sampling)
...
mt_exp= # Specify the directory path for ST experiment.
...
Options:
    ...
    --mt_exp           # Specify the directory path for ST experiment.
                       # If this option is specified, st_tag is ignored (default="${mt_exp}").
...
if [ -z "${mt_exp}" ]; then
    ...
    if [ "${framework}" = "huggingface" ]; then
        mt_exp="${expdir}/hf_${st_tag}${_suf}"
    else
        mt_exp="${expdir}/${st_tag}${_suf}"
    fi
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    log "Stage 2: Run (distributed) ST inference on the dev/test data."
    decode_suf="_org"
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    log "Stage 3: Run evaluation on the ST decoded data."
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    log "Stage 4: Run (distributed) MT-prompted ST inference on the dev/test data."
    decode_suf="_org"
    if "${merge_decode}"; then
        decode_suf="_merged"
    fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    log "Stage 5: Run evaluation on the MMT decoded data."
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    log "Stage 6: Run (distributed) MT inference on the dev/test data."
    decode_suf="_org"
    if "${merge_decode}"; then
        decode_suf="_merged"
    fi
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    log "Stage 7: Run evaluation on the MT decoded data."
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    log "Stage 8: Run (distributed) ASR inference on the dev/test data using the original model."
    decode_suf="_org"
    if "${merge_decode}"; then
        decode_suf="_merged"
    fi
    ...
    for dset in ${test_sets}; do
        ...
        _dir="${mt_exp}/${src_lang}/decode/vanilla/${dset}/asr/${decode_suf}"
...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        score_dir=scores_org/asr/hf_whisper_${model_name}/${src_lang}/vanilla${decode_suf}/${dset}
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
    log "Stage 10: Run (distributed) cascaded MT inference on the dev/test data."
    decode_suf="_org"
    if "${merge_decode}"; then
        decode_suf="_merged"
    fi
    ...
    for dset in ${test_sets}; do
        ...
        asr_dir="${mt_exp}/${src_lang}/decode/vanilla/${dset}/asr/${decode_suf}"
        _asr_hyp="${PWD}/${asr_dir}/text"
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
    log "Stage 11: Run evaluation on the cascaded MT decoded data."
